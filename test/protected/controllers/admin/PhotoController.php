...
//				'users'=>array('admin'),
	...
	public function actionCreate()
	{
		...
		{			
			...
			$photo = CUploadedFile::getInstancesByName('Photo');
//			var_dump(count($photo));exit();
			if(isset($photo) && count($photo)> 0) 
			{
//				var_dump($photo);exit();
				 foreach ($photo as $key=>$value) 
				 {
//				 	var_dump($value->filename);
				 	 if ($value->saveAs(Yii::getPathOfAlias('webroot').'/images/upload/'.$value->filename)) 
				 	 {				 	   
                        $model->setIsNewRecord(true);
                        $model->id = null;
                        $model->filename = $pic->filename;
                        $model->save();
                    }              
				 }
